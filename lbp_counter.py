...
def __get_all_face_image_paths_from_file_path(folder_path: str):
    """Searches for all pictures which are directly located in a given folder path and returns their path names in a
    list """
    image_list = []
    for file_name in listdir(folder_path):
        if (file_name.endswith(".png") or file_name.endswith(".jpg")) and file_name.endswith("face.jpg"):
...
def __process_cmd_line_parameter_and_create_image_path_list():
    ...
    all_image_paths = __get_all_face_image_paths_from_file_path(image_folder_path)
    log.log_info(str(len(all_image_paths)) + " face images where found.")
